...
# pure-math ("eager") implementation so we never hit MuDNN Flash kernels.
# ---------------------------------------------------------------------------
...
from rq_tokenizer import RQKMeansPlus, tokenizer_checkpoint_matches_model
...
def enable_gradient_checkpointing(model):
    """Enable gradient checkpointing on HSDBlock layers to save memory.

    Wraps each HSDBlock.forward so that during training, intermediate
    ...
    for module in raw.modules():
        if isinstance(module, HSDBlock):
            _wrap_block_for_checkpointing(module)
            count += 1
    if count > 0:
        print0(f"Gradient checkpointing enabled for {count} HSDBlock layers")


def _wrap_block_for_checkpointing(block):
    """Wrap a single HSDBlock's forward with torch.utils.checkpoint."""
    original_forward = block.forward

    def checkpointed_forward(x, token_types, seq_lens=None):
        if block.training:
            return grad_checkpoint(
                original_forward, x, token_types, seq_lens,
                use_reentrant=False,
            )
        return original_forward(x, token_types, seq_lens)
...
def train_mtp(model, train_loader, val_loader, cfg, tb: TBLogger,
              accum_steps=1, csv_log: CSVLogger = None):
    print0("\n" + "=" * 60)
    ...
    for epoch in range(cfg.train.mtp_epochs):
        ...
        if val_loss < best_val_loss:
            ...
            save_checkpoint(model, os.path.join(cfg.train.save_dir, "mtp_best.pt"))
...
def train_vaft(model, train_loader, val_loader, cfg, tb: TBLogger,
    ...
    for epoch in range(cfg.train.vaft_epochs):
        ...
        if val_loss < best_val_loss:
            best_val_loss = val_loss
            save_checkpoint(model, os.path.join(cfg.train.save_dir, "vaft_best.pt"))
...
def train_hepo(model, train_loader, val_loader, cfg, tb: TBLogger,
               extra_data=None, accum_steps=1, csv_log: CSVLogger = None):
    print0("\n" + "=" * 60)
    ...
    for epoch in range(cfg.train.hepo_epochs):
        ...
        for batch_idx, batch in enumerate(pbar):
            ...
            # Candidate generation (no gradient needed)
            model.eval()
            with torch.no_grad():
            ...
            if is_main():
                pbar.set_postfix(
                    policy=f"{policy_loss.item() if isinstance(policy_loss, torch.Tensor) else policy_loss:.4f}",
                    value=f"{value_loss.item():.4f}",
                    reward=f"{avg_reward:.4f}",
                )

            # Free intermediate tensors every batch in HEPO (memory-heavy stage)
            del gen_result_full, hepo_result, cand_codes, old_logprobs
            del pred_values, rewards, returns, advantages, ratio, clipped
            torch.musa.empty_cache()
        ...
        if val_loss < best_val_loss:
            best_val_loss = val_loss
            save_checkpoint(model, os.path.join(cfg.train.save_dir, "hepo_best.pt"))
...
def prepare_data(cfg, is_distributed=False):
    ...
    can_load = os.path.exists(tokenizer_path) and tokenizer_checkpoint_matches_model(
    ...
    if can_load:
        ...
        if os.path.exists(tokenizer_path):
            print0(
                "Existing tokenizer.pt does not match current TokenizerConfig "
                "(embed_dim, codebook_size, n_levels, or architecture). Retraining."
            )
            if is_main():
...
def main():
    parser = argparse.ArgumentParser(description="GPR Training Pipeline (DDP/MUSA)")
    ...
    parser.add_argument("--accum_steps", type=int, default=1,
                        help="Gradient accumulation steps (effective_bs = batch_size * accum_steps * world_size)")
    ...
    if is_main():
        ...
        with open(config_path, "w") as f:
            json.dump({
                ...
                "effective_batch_size": cfg.train.batch_size * args.accum_steps * world_size(),
    ...
    print0(f"Batch size per GPU: {cfg.train.batch_size}, "
           ...
           f"Effective: {cfg.train.batch_size * args.accum_steps * world_size()}")
    ...
    print0(f"Model moved to {device} ({dtype})")
    ...
    if args.stage in ("all", "mtp"):
        model = train_mtp(model, train_loader, val_loader, cfg, tb,
                          accum_steps=accum, csv_log=csv_log)
    ...
    if args.stage in ("all", "vaft"):
        if args.stage == "vaft" and args.resume is None:
            ...
            if os.path.exists(mtp_path):
                ...
                load_checkpoint(model, mtp_path)
    ...
    if args.stage in ("all", "hepo"):
        if args.stage == "hepo" and args.resume is None:
            ...
            if os.path.exists(vaft_path):
                ...
                load_checkpoint(model, vaft_path)
        model = train_hepo(model, train_loader, val_loader, cfg, tb,
                           extra_data=extra_data, accum_steps=accum,
                           csv_log=csv_log)
...
if __name__ == "__main__":
    try:
        ...
        rank_id = int(os.environ.get("RANK", -1))
        print(f"\n{'='*60}", file=sys.stderr, flush=True)
        print(f"[rank{rank_id}] FATAL ERROR:", file=sys.stderr, flush=True)
